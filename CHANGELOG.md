...
- **`--rt <file>`** — print one file's *render tree* as JSON: the Logical
  Printing Tree lowered through `Formatter.RenderTree`, which is what the
  renderer is actually handed. It is `--lpt` with the source positions gone,
  plus the four author-intent booleans that lowering computes
  (`sharesRowWithPrevItem`, `hasSourceContent`, `variantsSpanRows`,
  `typeSegmentsBroken`, reported per node in a `flags` string). Those four
  appear in no other dump — they are not in the LPT, and they have been consumed
  by the time there is a `Box` — so this is the only way to read one without
  re-deriving it from source rows by hand.

### Changed

- `--box`'s help text calls its output "the `Box` tree (the render plan)" rather

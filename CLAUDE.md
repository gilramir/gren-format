...
On the `geng` branch this is a Geng application, built by the fork in the
geng-lang checkout it is vendored into (geng-lang `m1a-fmt.md` §F18): `build.sh`
runs `vendor/gren-lang/compiler/app make Main` there, with `geng.toml`'s
`[sources]` pointing at the vendored library, argparse, `gren-diff` and
`compiler-common`. The fork has to have been built (`devbox run build` in
`vendor/gren-lang/compiler`). The app needs Node 22 or later.

```bash
cd gren-format && ./build.sh     # the fork's `make Main --output=app`
tests/run-tests.sh               # builds the CLI and its 92-test suite, runs it

./gren-format.sh --show MyFile.geng   # one file to stdout, with all checks
./app                                 # format the whole project in place
```

## Behaviour

`onCommand` in `src/Main.geng` dispatches in this order:
...
1. **Positional paths** → format those files/directories in place. A directory
   ...
   no-argument run takes its files from `geng.toml`, not from the tree).
...
3. **No arguments** → format every source file in the project in place; needs a
   `geng.toml` in the cwd or a parent.
